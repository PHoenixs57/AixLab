...
import type {
  DirListing, FileRead, PanelEnvelope, PanelError, SearchView,
} from '../core/types.ts'
...
export type PanelChangeEvent = { kind: 'fs' }

/**
 * Subscribe to host-pushed fs watch events for one project root. Reconnects
 * are handled by the EventSource; the caller re-subscribes when the root
 * changes.
