/**
 * The Explorer column: a 37px header bar with the collapse chevron, the
 * persistent filename search at the top (150ms debounced; a hit click REVEALS
 * the file in the tree — expand ancestors + select — never opens preview),
 * and the lazy file tree (34px rows, full-row expand/collapse, 16px icons).
 *
...
export function ExplorerPanel({
  ...
  return (
    <div className="aionui-root" style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      {/* The header bar with the in-column collapse chevron. */}
...
function FileTree({ stores }: { stores: PanelStores }): JSX.Element {
  ...
  if (entries === undefined) {
    return <div className={explorerCss.searchStatus}>{t('common.loading')}</div>
  }
