...
import { displayPresetLabel, FULL_ACCESS_PRESET } from './presentation.ts'
...
export function PermissionRow({ load, select, usePermission, t }: PermissionRowProps) {
  ...
  // Known machine values translate through the row's locale seat; host
  // labels for custom presets pass through unchanged.
  const labelOf = (option: PermissionSettingsState['options'][number]): string =>
    displayPresetLabel(option.id, option.label, t)
    ...
    <>
      <div className={css.row}>
        ...
        <Menu
          ...
          items={state.options.map(option => ({ id: option.id, label: labelOf(option) }))}
