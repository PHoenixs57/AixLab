...
The rows render through shared pure card models for `terminal`, `read`, `diff`, `search`, and `web` render intents. Unknown intent tags and malformed wire card data fall back to flattened Tool result text. (The upstream details-panel body was removed: the right details column hosts only the literature window, and tool rows keep their inline bodies.)
