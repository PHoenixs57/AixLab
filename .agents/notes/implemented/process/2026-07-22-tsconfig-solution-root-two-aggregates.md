...
- Adding an ordinary package registers it in exactly one aggregate's references: Host packages in `tsconfig.host.json` and Client packages in `tsconfig.client.json`. A package with separate Host and Client project files registers the Host leaf in the Host graph and the Client leaf in the Client graph; its package-root solution enters neither aggregate. `api/remotes` is the only split package whose Client leaf consumes a Host-generated `/remote` contract; other split packages use the same matching-face rule without that generated dependency.
