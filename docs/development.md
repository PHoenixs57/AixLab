...
The repository uses isolated Host and Client aggregates. An ordinary package is registered in exactly one aggregate: Host packages in `tsconfig.host.json` and Client packages in `tsconfig.client.json`. A package with separate Host and Client project files registers each leaf in the matching aggregate.
...
| `tsconfig.host.json` | Host aggregate: Host packages, examples, tests, scripts, website, and Host leaves of split packages, including the exceptional Host project of `api/remotes`. | Yes |
| `tsconfig.client.json` | Client aggregate: `packages/client/*` packages and their tests, `apps/web`, and Client leaves of split packages, including the exceptional Client project of `api/remotes`. | Yes |
...
- A new ordinary package is registered in exactly one aggregate. Having both a Node loader entry and a browser entry is not a reason to split a package; an ordinary Client plugin produces both runtime artifacts during the Client build phase.
- A package with separate `tsconfig.host.json` and `tsconfig.client.json` files is registered through the Host leaf in the Host graph and the Client leaf in the Client graph. The package-root `tsconfig.json` remains only a solution, and each reference names the leaf for its compiler face.

`api/remotes` is the only split package whose Client entry imports `/remote` declarations that Host tsdown must generate first. The workspace `constraints` gate walks the reachable Project Reference graph and checks each referencing project's own compiler face: a single-config target remains valid from either face, while a split target must name the matching leaf rather than its solution root or opposite leaf; it discovers split packages from the presence of both leaf configs, so each split joins the gate automatically. The [`api-remotes` README](../packages/api/remotes/README.md) explains its Host/Client build order.
