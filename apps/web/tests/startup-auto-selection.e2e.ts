...
describe('web e2e: startup auto-selection', () => {
  ...
  it('keeps the resident Hero and composer nodes when the first Workspace session appears', async () => {
    ...
    const headline = page.getByText('Start the Research Journey', { exact: true })
    const fish = headline.locator('xpath=preceding-sibling::span[1]/img')
    const fishHitbox = fish.locator('..')
    expect(await fish.getAttribute('src')).toBe('/microscope.png')
  ...
  it('keeps the hero and the composer on screen while the auto-selected blank session opens', async () => {
    ...
    expect(await page.getByText('Start the Research Journey').isVisible()).toBe(true)
